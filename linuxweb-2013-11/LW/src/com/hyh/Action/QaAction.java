...
public class QaAction extends ActionSupport{
	...
	public String execute()throws Exception{
		List list=fd.findQaByPage(PageNow, PageSize);
		...
		for(int i=list.size()-1;i>=0;i--){
			...
			files.setIntroduct(file.getQuestion());
			files.setApprove(file.getApprove());
			files.setCommentno(file.getAnserno());
		...
		if(fd.findQaCount("","index")%PageSize>0){
			TolePage=fd.findQaCount("","index")/PageSize+1;
		}else TolePage=fd.findQaCount("","index")/PageSize;
